...
> **타깃 보정(정직):** *발표 우선* — BoB / 금융보안 세미나 / AWS Summit 커뮤니티 토크는 **지금 가능**.
> *논문*은 **국내 응용/워크숍**(KIISC CISC-S/W·CISC-W, 금융보안원/KISA 기술트랙)이 현실 타깃이며,
> [검증가능성 커버리지 평가](../docs/evaluation-coverage.md)가 그 바를 넘기는 핵심. **탑티어(USENIX/CCS)는
> 과장하지 말 것** — 신규 메커니즘 없는 통합·규제매핑 기여.
