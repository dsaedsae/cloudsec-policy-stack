...
종료하고, `scripts/scan.sh`는 `set -euo pipefail`이라 그 비제로에서 즉시 멈춘다([`scripts/scan.sh`](https://github.com/dsaedsae/cloudsec-policy-stack/blob/master/scripts/scan.sh)
는 콘솔 요약을 살리려 `terraform`·`k8s`를 두 번에 나눠 돈다). CI도 같은 게이트다 —
[`.github/workflows/ci.yml`](https://github.com/dsaedsae/cloudsec-policy-stack/blob/master/.github/workflows/ci.yml)의 `policy` 잡은 한 줄
