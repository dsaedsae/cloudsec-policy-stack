...
> 그대로 연결한다. 헤드라인 82.5%(33/40)는 **이 문서로 인해 바뀌지 않는다** — 이 문서는 메트릭을 더하지 않고,
> 같은 증거를 다른 렌즈로 본다.
...
| **DETECT** (오작동 NHI 포착) | 런타임 exec 즉시 SIGKILL(data tier zero-exec) | ED1 | ✅ VERIFIED |
...
이 문서는 **CSV에 행을 더하지 않고, 헤드라인 82.5%(33/40)를 이 문서로 인해 바꾸지 않는다.** 위 표의 모든 id
