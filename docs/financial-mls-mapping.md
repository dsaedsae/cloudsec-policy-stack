...
| **이상 탐지 (EDR)** | "분리로 침투 자체가 어려움" | **Tetragon**(eBPF) 데이터 티어 **나이브 직접 셸-명 execve** in-kernel 차단(renamed-binary/execveat/fd-exec 우회 가능 — 강건한 답은 zero-exec(execve+execveat)/distroless 이미지, M8 라이브 검증; THREAT_MODEL·M8 참조). 프로세스 감사(audit)=CONFIGURED(스위트 미단언) | Tenet 5·7 | 2.11 사고대응/탐지 | `naive shell exec→SIGKILL 137` |
