...
| 축 | 내용 |
|----|------|
| **데이터 등급** | **기밀(C, Classified) · 민감(S, Sensitive) · 공개(O, Open)** 3등급, 등급별 **차등 보안통제**, 시스템은 **최상위 등급 기준**. ⚠️ 이 C/S/O 등급체계는 **국정원(NIS) 공공분야 다층보안(MLS)** — 민·관 합동 망보안정책 개선 TF — 에서 정의됐고, FSC 금융 로드맵이 같은 모델을 *적용*하는 흐름이다(금융권 최종 등급정의는 전자금융감독규정 개정으로 구체화). 본 데모의 db/api/web C/S/O는 그 모델을 *차용한 가정*. |
