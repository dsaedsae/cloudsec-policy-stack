...
| **암호화 강화 (저장)** | "내부 저장소 접근자 제한으로 충분" | etcd **Secret AES-CBC 암호화**(`k8s:enc:aescbc`) | Tenet 1 (800-207엔 at-rest 전용 원칙 없음) | 2.7 암호화 | raw etcd 암호문/평문0 |
...
- **WireGuard는 노드 간 암호화다.** 이 데모는 **2워커** 구성이고 `db`를 `api`와 다른 노드로
  ...
  *WireGuard 활성 + api/db 다른 노드*를 함께 단언해 "이 앱 홉이 선상에서 암호화됨"을 증명한다.
  단, 노드배치+`encrypt status`로 추론(Cilium 설계상 크로스노드 트래픽은 암호화)할 뿐 **tcpdump
  패킷캡처는 미수행**(더 강한 증거, 커버리지에서 CONFIGURED로 분류).
