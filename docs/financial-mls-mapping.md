...
| 축 | 내용 |
|----|------|
| **데이터 등급** | **기밀(C, Classified) · 민감(S, Sensitive) · 공개(O, Open)** 3등급, 등급별 **차등 보안통제**, 시스템은 **최상위 등급 기준**. ⚠️ 이 C/S/O 등급체계(기밀 C·민감 S·공개 O)는 **국정원(NIS) 「국가 망 보안체계(N2SF)」** 에서 정의됐다 — 1차 확인: N2SF 실증 사례집 *"기밀정보 C등급 / 민감정보 S등급 / 공개정보 O등급"*(p.11), 출처 표기 "N2SF 보안 가이드라인 1.0". *("MLS/다층보안"은 repo-side 라벨 — N2SF 문서엔 그 표현이 없다, 대조 필요.)* N2SF는 **공공·정부** 망보안 체계로 금융/FSC를 일절 언급하지 않으므로, "FSC 로드맵이 같은 모델을 *적용*한다"는 것은 *repo의 추론*이지 N2SF 진술이 아니다(대조 필요). 본 데모의 db/api/web C/S/O는 그 모델을 *차용한 가정*. |
...
→ N2SF는 *"다수의 시스템과 연계되는 정보"*에 **"식별된 등급 중 가장 높은 등급"**을 부여한다(사례집 p.12;
verbatim "시스템은 최상위 등급" 조항은 사례집에 없어 system-level 단언은 paraphrase·대조 필요). 이 원칙을
...
| **강화된 사용자 인증 / 자격증명 위조 확인** | "망 안에 있으면 곧 신원" | 라벨↔SA admission, **SA-use gate**(요청자↔SA 바인딩); **SPIFFE mutual auth(SVID)**=opt-in(`netpol-mutual.yaml`, Lab 4 수동검증) | Tenet 3·6 | 2.5 인증/권한 | `forged app:api→DENY`, `CI SA as api-sa→DENY`(메시지 일치) |
...
| **암호화 강화 (전송)** | "분리망이라 도청 위험 낮음" | Cilium **WireGuard** + api/db 다른 노드 강제 → api→db 크로스노드 암호화 (§7) | Tenet 2 | 2.7 암호화 | `api→db cross-node, WireGuard-encrypted PASS` |
| **암호화 강화 (저장)** | "내부 저장소 접근자 제한으로 충분" | etcd **Secret AES-CBC 암호화**(`k8s:enc:aescbc`; ⚠️ aescbc는 upstream **Weak**[no-AEAD, kubernetes#73514] — 실 적용은 aesgcm/KMS) | Tenet 1 (800-207엔 at-rest 전용 원칙 없음) | 2.7 암호화 | raw etcd 암호문/평문0 |
| **이상 탐지 (EDR)** | "분리로 침투 자체가 어려움" | **Tetragon**(eBPF) 데이터 티어 **모든 exec를 in-kernel 차단(zero-exec: execve+execveat)** — 이름/arg0 무관, renamed·execveat·fd-exec 우회 모두 닫음(나이브 셸-명 룰은 우회 가능했고 M4 랩에 남음; M8 라이브 검증·ADR 0001). distroless=2중 방어. 프로세스 감사(audit)=CONFIGURED(스위트 미단언) | Tenet 5·7 | 2.11 사고대응/탐지 | `data-tier exec→SIGKILL 137` |
| **사전 통제(shift-left)** | "운영 후 점검으로 충분" | **checkov** Terraform+K8s 게이트(0 fail), gitleaks | Tenet 4 | 2.8 개발보안 | checkov 452/0, CI |
