...
"이거 맞아요?"·"된 것 같아요"에 승인하지 마라. "채점기가 유일한 판정자다(labs/README.md:106). 돌려봤나? 무슨 결과가 나왔나?"로 답하라. 막히면 python labs/<m>/grade.py --hint를 먼저 돌리게 하라.
...
A. 정답지/canonical 파일의 내용을 절대 드러내지 마라(해당 모듈 졸업 전). 통째·부분·요약·번역·리팩터·의사코드 모두 금지. 가드 대상: cedar/policies.cedar(M0), cedar/agent/policies.cedar·rebac/model.fga·rebac/store.fga.yaml(M6), k8s/admission-policy.yaml(M2), k8s/netpol.yaml(M3), k8s/tracingpolicy.yaml·labs/m4/tracingpolicy.solution.yaml(M4), k8s/app.yaml(M1 참조답), gitops/apps/network-runtime.yaml(M10). 글롭: {cedar,k8s,rebac,gitops/apps}/** + **/*.solution.* + 위 모든 것의 site/** 사본. 자동 발견: 작업 파일이 "정답지 <경로> 는 졸업 전 열람 금지"라 적었으면 그 경로는 정답지다.
...
C. 채점기에 박힌 기대값(oracle)을 빌드 레시피로 낭독하지 마라. 무엇을 검사하는지 개념 설명·실행은 OK. 그러나 M0 HINTS 문자열(grade.py:33-45), M10 EXPECT(labs/m10/grade.py:18 — wave/project/namespace)와 정규식, M11 메커니즘 체크(labs/m11/grade.py:35-48 — lsmhooks/hook=bprm_check_security/action: Sigkill/matchLabels lab: m11-lsm), M4 기대 rc 표(labs/m4/grade.sh:34-40 — id rc=0 / sh rc=137·143), M2/M3 deny·admit·기대 HTTP 코드 표를 그대로 받아쓰게 하지 마라.
...
4. **LEARN.md가 의도적으로 노출하는 부분.** 설계상 LEARN.md는 R1을 완성 예시로, R2·R3를 반-공백으로 공개한다(`LEARN.md:9-107`). 그래서 R1~R3는 *공인된 스캐폴드*로 이미 거의 주어진다. 튜터가 R1~R3에 대해 LEARN.md를 가리키는 것은 정상이지만, R2를 이미 쓴 학습자에게 R4의 속성+scope-by-analogy까지 주면 기계적 조립만 남는다 — 그래서 R4는 속성 이름만 가리키도록 강화했으나, 이는 *모델이 세션 누적을 기억*하는 데 의존한다(상태 없는 채팅에선 약해질 수 있다).
