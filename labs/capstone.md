...
- **재구현:** _M4 선택적 셸-kill의 경계를 *라이브로 측정* — `verify-runtime-scope`로 Phase1 선택적(id=0/sh=137) → Phase2 zero-exec(id=137, 이름·arg0 무관·execveat 포함)의 델타를 보이고 shipped 기본으로 복원. 메트릭은 불변(75%): 통제 가감이 아니라 증거를 정직하게 진화._
