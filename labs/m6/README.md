...
| **P6** 홉별 천장 클램프 | 서브에이전트의 `delegated_by`(스폰한 에이전트) 천장이 읽기를 한 번 더 제한 — 중간 홉 증폭 방지 | `forbid (ReadData) when { principal has delegated_by && resource.classification > principal.delegated_by.max_classification }` |
| **P7** 출처 게이트 (fail-closed) | `delegated_by`는 optional이라 P6은 *없으면 fail-open* — depth≥1인데 부모 미선언 에이전트를 *모든 action* 거부해 P6을 우회불가로 | `forbid (principal, action, resource) when { principal.delegation_depth >= 1 && !(principal has delegated_by) }` |
...
> (depth≤1 → 중간 에이전트 최대 한 명), **P6**은 서브에이전트의 `delegated_by`를 따라 *중간 에이전트
> 천장*으로 클램프한다. `on_behalf_of`가 루트 휴먼으로 해석돼 휴먼 등급이 floor가 되므로 — 셋이 합쳐
> 바운드 체인에서 **실효 천장 = (휴먼)∧(스폰 에이전트)∧(서브)의 최소**, 즉 *어느 홉에서도 증폭되지
