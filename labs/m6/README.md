...
## Part A — Cedar ABAC 교집합 (목표 12/12)

```powershell
.venv\Scripts\python.exe labs\m6\grade.py agent     # 시작: 6/12 (default-deny만)
```

`labs/m6/agent-policies.cedar`의 P1–P4 TODO를 채워라. 스키마는 `cedar/agent/schema.json`(열람 OK).
...
> **반증가능성(이 repo의 핵심 명제):** 12개 시나리오 중 두 개는 *일부러* P2·P3가 **유일한 결정자**가
> 되도록 설계됐다("P2 IS LOAD-BEARING", "P3 IS LOAD-BEARING" — Allow 기대). 12/12를 만든 뒤,
> P2를 통째로 지워보라 → "P2 IS LOAD-BEARING"이 Allow→Deny로 *뒤집힌다*. P3도 마찬가지. **뒤집히지
...
```powershell
.venv\Scripts\python.exe labs\m6\grade.py           # 12/12 + 11/11 → M6 GRADUATED
```
...
- [ ] `grade.py` **12/12 + 11/11**
- [ ] P2를 지우면 "P2 IS LOAD-BEARING"이, P3를 지우면 "P3 IS LOAD-BEARING"이 뒤집히는 걸 직접 확인
