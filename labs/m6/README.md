...
## Part A — Cedar ABAC 교집합 (목표 14/14)

```powershell
.venv\Scripts\python.exe labs\m6\grade.py agent     # 시작: 7/14 (default-deny만)
```

`labs/m6/agent-policies.cedar`의 P1–P5 TODO를 채워라. 스키마는 `cedar/agent/schema.json`(열람 OK).
...
> **반증가능성(이 repo의 핵심 명제):** 14개 시나리오 중 셋은 *일부러* P2·P3·P5가 **유일한 결정자**가
> 되도록 설계됐다("P2 IS LOAD-BEARING", "P3 IS LOAD-BEARING", "ASI08 P5 IS LOAD-BEARING"). 14/14를
> 만든 뒤, P2를 통째로 지워보라 → "P2 IS LOAD-BEARING"이 Allow→Deny로 *뒤집힌다*. P3도, **P5도** 마찬가지
> (P5를 지우면 depth-2 에이전트가 *공개 데이터*를 읽는 케이스가 Deny→Allow로 뒤집힌다). **뒤집히지
> 않으면** 그 정책은 죽은 코드다(테스트가 그걸 증명 못 함). 이게 "통과한다 ≠ 옳다"의 결정판 —
> M0 Step 3의 뮤테이션 교훈과 같은 것.

> **ASI08이 왜 새 정책인가:** 에이전트가 다른 에이전트를 *스폰*하는 멀티에이전트 시스템에서, 권한이
> 체인을 따라 무한 cascade하면 안 된다(OWASP Agentic Top 10 2025-12-09). P5는 *체인 길이*를 묶고,
> P2의 교집합은 `on_behalf_of`가 항상 **루트 휴먼**으로 해석되므로 *어느 홉에서도 권한이 증폭되지
> 않게* 한다. 둘이 합쳐 cascading-delegation 통제다. 배경: [authorization-model §5](../../docs/authorization-model.md).
...
```powershell
.venv\Scripts\python.exe labs\m6\grade.py           # 14/14 + 11/11 → M6 GRADUATED
```
...
- [ ] `grade.py` **14/14 + 11/11**
- [ ] P2를 지우면 "P2 IS LOAD-BEARING"이, P3를 지우면 "P3 IS LOAD-BEARING"이, P5를 지우면 "ASI08 P5 IS LOAD-BEARING"이 뒤집히는 걸 직접 확인
