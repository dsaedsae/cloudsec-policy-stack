...
> 헤드라인 **77.5%(31/40)는 이 모듈로 바뀌지 않는다.** 추가 후보는 단 1행 — **IN1**(새 family `integrity`,
> "drift auto-reverted within sync interval") — 이고, 그조차 정직하게: CI hard-gate면 VERIFIED(→78.0%), 로컬
> 랩 grader만이면 CONFIGURED(→**75.6%**, denominator +1이 비율을 *먼저* 떨어뜨림 — ID8의 "증명 전엔 claim 안 함"
