...
[라이브 검증 21/21](docs/evaluation-coverage.md) · [검증가능 커버리지 75%](docs/evaluation-coverage.md) · [적대적 검증: CRITICAL 1 발견·수정](THREAT_MODEL.md) · 무료 로컬 $0
...
- **검증가능성 기준 + 커버리지 측정** — "각 규제 요구는 시행을 증명하는 실행 테스트에 대응돼야 한다"를 기준으로, MLS 보상통제의 75%가 코드로 검증가능함을 정량화(갭 공개). [→ 평가](docs/evaluation-coverage.md)
...
| 항목 | 결과 |
|------|------|
| **검증가능성 커버리지 (정량 헤드라인)** | 워크로드 적용가능 MLS 보상통제 sub-requirement의 **75% (30/40)** 가 *코드로 검증* — 갭(CONFIGURED 6·NOT-COVERED 4·GOVERNANCE 2)을 정직하게 공개. [→ 평가](docs/evaluation-coverage.md) |
