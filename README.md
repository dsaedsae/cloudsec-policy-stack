...
- ✅ **Re-implementation track (`labs/`)** — self-graded learn-by-rebuilding: strip a control to a skeleton, rewrite it from the spec, and the existing test/verify harness grades you (the verifiability thesis doubled as an autograder). M0 (Cedar authz, no cluster: core 8 + ext 3 = 11/11 with an over-grant catch and a boundary/mutation lesson) is live; M1–M6 follow the module ladder in `labs/README.md`.
