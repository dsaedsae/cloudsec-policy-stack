...
- ✅ **WireGuard packet capture (opt-in evidence)** — `scripts/capture-wg.*` captures on the db node's host netns, gated on app traffic actually flowing: ≥40 WireGuard packets (UDP/51871) cross the wire and **0 plaintext** bytes appear on `eth0`. The dispositive evidence is WG-packet presence + cross-node placement; plaintext-absence is corroborating (cross-node pod traffic is encapsulated). Upgrades ET2 to VERIFIED (gated/evidence, not a baseline check).
